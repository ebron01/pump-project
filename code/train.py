...
def main():
    ...
    l_mtlr = LinearMultiTaskModelSkl(structure = [ {'activation': 'relu', 'num_units': 128}, 
                          {'activation': 'tanh', 'num_units': 128}],bins=400, auto_scaler=True,num_epochs=10000)
    l_mtlr.fit(X_train, Y_train, lr=1e-5,optimizer='rmsprop')
    display_loss_values(l_mtlr.pysurvival_model)
    integrated_brier_score(l_mtlr.pysurvival_model, X_test, Y_test[time_column], Y_test[event_column], t_max=120,
                       figure_size=(20, 6.5) ) #0.08
    # pred = l_mtlr.predict(X_test,[1])
    # l_mtlr.predict(X_test, all_data_train['run_days']+2)
